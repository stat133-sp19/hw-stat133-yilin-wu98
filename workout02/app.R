...
server <- function(input, output) {
  ...
  mode_in<- reactive({ 
    ...
    for (i in 0:input$year){
      mode_i[i+1,'year'] <- i
      mode_i[i+1,'no_contrib'] <- future_value(amount=input$initial_amount,rate=input$return_rate/100,years=i)
      mode_i[i+1,'fixed_contrib']<-future_value(amount=input$initial_amount,rate=input$return_rate/100,years=i)+annuity(contrib=input$annual_contrib,rate=input$return_rate/100,years=i)
      mode_i[i+1,'growing_contrib']<-future_value(amount=input$initial_amount,rate=input$return_rate/100,years=i)+growing_annuity(contrib=input$annual_contrib,rate=input$return_rate/100,growth=input$growth_rate/100,years=i)
    }
    mode_i})
  
  mode_i_full <- reactive({ mode_1 <- data.frame()
  mode_1[1:(input$year+1),'value']<-mode_in()[1:(input$year+1),'no_contrib']
  mode_1[1:(input$year+1),'year']<-mode_in()[1:(input$year+1),'year']
  mode_1[1:(input$year+1),'variable']<-'no_contrib'
  mode_1[(input$year+2):(2*input$year+2),'year']<-mode_in()[1:(input$year+1),'year']
  mode_1[(input$year+2):(2*input$year+2),'value']<-mode_in()[1:(input$year+1),'fixed_contrib']
  mode_1[(input$year+2):(2*input$year+2),'variable']<-'fixed_contrib'
  mode_1[(2*input$year+3):(3*input$year+3),'year']<-mode_in()[1:(input$year+1),'year']
  mode_1[(2*input$year+3):(3*input$year+3),'value']<-mode_in()[1:(input$year+1),'growing_contrib']
  ...
  output$plot <- renderPlot({
    if (input$facet=='No'){
      ...
      ggplot(mode_i_full(),aes(color=variable))+
        ...
        facet_grid(.~variable)+
        # scale_color_identity(labels=c('growing_contrib','no_contrib','fixed_contrib'),guide='legend',name='mode_i')+
        theme(legend.text=element_text(size=rel(1.2)))+
